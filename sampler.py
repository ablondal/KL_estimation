...
def RB_kl(length, reps, temp, folder, outp):
    kls = []
    kl_sum = 0
    kl_num = 0
    for k in range(reps):
    ...
    with open(folder+outp+'.json', 'w') as f:
        json.dump(kls, f, indent=2)

def run_smc(func, length, reps, temp, folder outp):
...
t_length = 20
temperature = 0.4
output_folder = 'tests2/'

######### MODE 1: RUN THE FUNCTIONS ENUMERATED IN THE COMMAND LINE ##########
