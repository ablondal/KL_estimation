...
class LongTailedDistribution:
    ...
    def zipfian_with_cutoff(vocab_size, alpha=1.5, cutoff_rank=100):
        """
        Power law head + exponential tail cutoff
        Models real LLMs that have finite context effects
        """
        ranks = np.arange(1, vocab_size + 1)
        probs = 1.0 / (ranks ** alpha)
    ...
    def perturb_distribution(base_probs, shift_ratio=0.2, n_shift=100):
        """
        Create similar but different distribution by shifting probability mass
        Simulates fine-tuning or domain adaptation effects
        """
...
def create_model_pair(vocab_size=1000, divergence='medium'):
    """
    Create P (base) and Q (fine-tuned) using simple Zipfian distributions
    Args:
...
def create_model_pair_zipCutoff(vocab_size=2000, divergence='medium'):
    """
    Create P and Q using Zipfian with exponential cutoff
    Models LLMs with finite context effects
    """
...
def create_model_pair_mixture(vocab_size=2000, divergence='medium'):
    """
    Create P and Q using complex mixture distributions
    Most realistic model of actual LLM token distributions
    """
...
def compute_proposal_balanced2 (P_probs, Q_probs, eps=1e-12):
    ...
    """
    Balanced variant 2: r ∝ √(PQ·(log²(P/Q) + 1))
    Quadratic divergence weighting
    """
...
def compute_proposal_balanced1 (P_probs, Q_probs, eps=1e-12):
    ...
    """
    Balanced variant 1: r ∝ √(PQ)·|log(P/Q)|
    Simple geometric mean with linear divergence weighting
    """
...
def compute_proposal_balanced(P_probs, Q_probs, eps=1e-12):
    """
    Balanced proposal: r ∝ √(P·|log(P/Q)|·Q/(P+Q))
    Balances P's coverage with Q's support and divergence weighting
    Normalized by sum to avoid extreme weights
    """
    P_safe = np.clip(P_probs, eps, 1.0)
    Q_safe = np.clip(Q_probs, eps, 1.0)
...
def compute_proposal_cross_entropy(P_probs, Q_probs, temperature=0.5, eps=1e-12):
    """
    Cross-entropy tilted proposal: r ∝ P·exp(t·H(P,Q))
    where H(P,Q) = -P log Q
    Targets tokens with high negative log-probability under Q
    """
    P_safe = np.clip(P_probs, eps, 1.0)
    Q_safe = np.clip(Q_probs, eps, 1.0)
...
def compute_proposal_chi2_aware(P_probs, Q_probs, alpha=0.5, eps=1e-12):
    """
    Chi-square aware proposal: r ∝ P·(P/Q)^α
...
def compute_proposal_exponential_family(P_probs, Q_probs, beta=0.5, eps=1e-12):
    """
    Exponential family tilt: r(x) ∝ P(x)·exp(β·|log(P/Q)|)
    Exponentially boosts high divergence regions
    Aggressive targeting for large β
    """
    P_safe = np.clip(P_probs, eps, 1.0)
    Q_safe = np.clip(Q_probs, eps, 1.0)
...
def compute_proposal_adaptive_mixture(P_probs, Q_probs, eps=1e-12):
    """
    Adaptive log-mixture: r ∝ P·(1 + |log(P/Q)|)
    First-order Taylor approximation of exponential tilting
    More weight on high-divergence when differences are large
    """
    P_safe = np.clip(P_probs, eps, 1.0)
    Q_safe = np.clip(Q_probs, eps, 1.0)
...
def compute_proposal_alpha_mixture(P_probs, Q_probs, alpha=0.5, eps=1e-12):
    """
    Alpha-mixture: r(x) = α·P(x)|log(P/Q)| + (1-α)·P(x)
    Interpolates between P and divergence-weighted P
    """
    P_safe = np.clip(P_probs, eps, 1.0)
    Q_safe = np.clip(Q_probs, eps, 1.0)
...
def compute_proposal_optimal(P_probs, Q_probs, eps=1e-12):
    """
    Optimal (theoretical ceiling): r ∝ P|log(P/Q)|
...
def importance_sampling_estimator(P_probs, Q_probs, n_samples=100, alpha_values=[0.3, 0.5, 0.7, 0.9], eps=1e-12):
    ...
    Generates proposals and computes weighted estimates
    """
    r_probs_list = []
